...
extern "C" void app_main(void) {
    ...
    // 1. Inicializar LEDs RTC
    leds_init();

    // 2. Computar Geofence (Bounding Box)
    ...
    // 3. Actualizar LEDs con retención RTC
    leds_set_geofence_state(inside);

    // 4. Inicializar LoRa y transmitir heartbeat
    if (lora_init()) {
        lora_send_heartbeat(inside);

        // 5. Esperar downlink del Gateway
        lora_wait_for_downlink();

        // Si cambió la posición o el geofence, re-evaluar antes de dormir
        rtc_state_get_fence(&min_lat, &max_lat, &min_lon, &max_lon);
        bool new_inside = geofence_is_inside(rtc_state_get_lat(), rtc_state_get_lon(),
                                             min_lat, max_lat, min_lon, max_lon);
        if (new_inside != inside) {
            ESP_LOGI(TAG, "Estado de geofence cambió tras downlink, actualizando LEDs");
            leds_set_geofence_state(new_inside);
