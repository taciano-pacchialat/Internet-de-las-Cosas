...
La representación gráfica del terreno y de las unidades ganaderas se ejecuta mediante la biblioteca de \textit{Leaflet.js}. Al iniciar la sesión, el cliente realiza una petición HTTP inicial hacia el endpoint \texttt{GET /api/fence/current} para obtener el cerco activo de la base de datos y trazar el polígono delimitador sobre la capa del mapa.
...
Al finalizar el ajuste, el operador acciona el comando de guardado, lo que desencadena la emisión de una petición \texttt{POST /api/update-fence}. El servidor recibe la nueva geometría, recalcula la caja (\textit{Bounding Box}) y actualiza en milisegundos las reglas de validación en todos los nodos de monitoreo activos.
