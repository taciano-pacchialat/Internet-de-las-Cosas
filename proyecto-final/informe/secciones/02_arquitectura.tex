...
\begin{tikzpicture}[
    ...
    % Nodos
    \node[block] (edge) {\textbf{Módulo Borde}\\ESP32\\+ SX1278};
    \node[block, right=of edge] (gateway) {\textbf{Módulo Gateway}\\ESP32\\+ Radio SX1278};
    \node[cloud, right=of gateway] (broker) {\textbf{Broker MQTT}\\Mosquitto (Docker)\\\footnotesize\texttt{geofence/heartbeat}};
    
    \node[server, below=0.7cm of broker] (nodered) {\textbf{Node-RED}\\(Docker)\\Alertas};
    \node[server, left=of nodered] (influx) {\textbf{Base de Datos}\\InfluxDB\\\footnotesize\texttt{geofence\_config}};
    \node[server, below=0.7cm of nodered] (ui) {\textbf{Interfaz Web \& Bot}\\Dashboard / Telegram};

    % Conexiones
    \draw[arrow] (edge) -- node[above, font=\footnotesize] {LoRa 433~MHz} node[below, font=\footnotesize] {} (gateway);
    \draw[arrow] (gateway) -- node[above, font=\footnotesize] {Wi-Fi / TCP} node[below, font=\footnotesize] {MQTT} (broker);
...
\textbf{MCU Borde} & ESP32-WROOM-32D & Dual-core & Procesamiento en collar, temporización con reloj de tiempo real (RTC) en Deep Sleep. \\ \addlinespace
\textbf{MCU Gateway} & ESP32-WROOM-32D & Dual-core & Receptor base de interrupciones de radio e interfaz Wi-Fi hacia el servidor local. \\ \addlinespace
