...
static esp_err_t handler_post_networks(httpd_req_t *req)
{
    ...
    if (cJSON_IsNumber(channel) && channel->valueint > 14) {
        cJSON_Delete(json);
        return send_error(req, 400, "5 GHz networks are not supported by ESP32");
    }
...
static esp_err_t handler_post_connect(httpd_req_t *req)
{
    ...
    if (req->content_len > 0) {
        ...
        if (json) {
            ...
            if (cJSON_IsString(ssid_item)) {
                esp_err_t ret = wifi_manager_connect(ssid_item->valuestring);
                cJSON_Delete(json);

                if (ret != ESP_OK) {
                    return send_error(req, ret == ESP_ERR_NOT_FOUND ? 404 : 400,
                                      ret == ESP_ERR_NOT_FOUND ? "Network not found" : "Failed to start connection");
                }
    ...
    if (ret != ESP_OK) {
        return send_error(req, 400, "Failed to start auto-connect");
    }
    return send_accepted(req);
}
...
static const char *simple_page_html =
    ...
    "scanMap={};let h='';d.networks.forEach(n=>{scanMap[n.ssid]=n;"
    "h+='<div class=\"net\" onclick=\"pickNetwork(\\''+n.ssid+'\\')\">'+"
    "n.ssid+' (ch '+n.channel+', '+n.rssi+' dBm)</div>';});"
    "document.getElementById('nets').innerHTML=h||'<p>No networks</p>';"
    "}catch(e){document.getElementById('nets').innerHTML='<p>Scan failed</p>';}}"
    "function pickNetwork(ssid){const n=scanMap[ssid];if(!n)return;document.getElementById('ssid').value=n.ssid;"
    "if(n.channel>14){msg('La red '+n.ssid+' está en canal '+n.channel+' (5 GHz). Un ESP32 solo conecta a 2.4 GHz.',0);return;}"
    "msg('Selected '+n.ssid+' (ch '+n.channel+')',1);}"
    "async function connect(){"
    "const s=document.getElementById('ssid').value;"
    "const p=document.getElementById('pass').value;"
    "const n=scanMap[s];"
    "if(!s){msg('Enter SSID',0);return;}"
    "if(n&&n.channel>14){msg('No se puede conectar a '+s+': es una red de 5 GHz.',0);return;}"
    "try{const add=await fetch(API+'/networks',{method:'POST',headers:{'Content-Type':'application/json'},"
    "body:JSON.stringify({ssid:s,password:p,priority:10,channel:n?n.channel:0})});"
    "if(!add.ok){msg('No se pudo guardar la red',0);return;}"
...
esp_err_t wifi_mgr_http_init(void)
{
    ...
    if (!g_wifi_mgr->httpd) {
        ...
        config.max_uri_handlers = 48;  // API + WebUI + captive + margin
